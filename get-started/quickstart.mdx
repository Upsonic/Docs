---
title: "Quickstart"
sidebarTitle: "Quickstart"
description: "Let's jumpstart your AI agent development"
hideToc: true
---

Get started with Upsonic by building your first agent in minutes.

The Quickstart guide walks you through the essential features of Upsonic, from creating your first agent to adding memory, knowledge bases, and building multi-agent teams. Each section provides practical examples you can run immediately.

## Your First Agent

### What is an Agent?

An Agent is an autonomous AI entity that can understand tasks, use tools, and make decisions to achieve goals. Think of it as an AI assistant with specific capabilities and expertise.

### Why use Agents?

Agents enable you to:
- **Automate complex workflows**: Let AI handle multi-step processes autonomously
- **Leverage external data**: Connect to APIs, databases, and tools for real-time information
- **Scale expertise**: Create specialized AI workers for different domains (research, writing, analysis, etc.)

Let's create your first agent with Upsonic by creating an Agent, Task, and Tool.

```bash
uv pip install upsonic
# pip install upsonic
```

```python stock_analysis.py
from upsonic import Agent, Task
from upsonic.tools.common_tools import YFinanceTools

agent = Agent(model="openai/gpt-5", name="Stock Analyst Agent")

task = Task(description="Give me a summary about tesla stock with tesla car models", tools=[YFinanceTools()])

agent.print_do(task)
```

## Add Memory

### What is Memory?

Memory allows your agents to remember past interactions and maintain context across multiple conversations. It's like giving your agent a notebook to track what happened before.

### Why use Memory?

Memory enables you to:
- **Maintain context**: Your agent remembers previous conversations and decisions
- **Personalize interactions**: Build on past exchanges for more relevant responses
- **Track state**: Keep important information throughout a session or across sessions

Add conversational memory to your agent to remember past interactions.

```python with_memory.py
from upsonic import Agent, Task
from upsonic.storage import Memory, InMemoryStorage

memory = Memory(
    storage=InMemoryStorage(),
    session_id="session_001",
    full_session_memory=True
)

agent = Agent(model="openai/gpt-5", memory=memory)

task1 = Task(description="My name is John")
agent.print_do(task1)

task2 = Task(description="What is my name?")
agent.print_do(task2)  # Agent remembers: "Your name is John"
```

<a href="/concepts/memory/overview">
<button type="button" className="px-5 flex items-center font-medium text-sm rounded-full py-2 shadow-sm text-white dark:text-gray-900 bg-primary-dark dark:bg-primary-light hover:opacity-[0.9] hover:bg-primary justify-center">
Learn Details

<svg className="size-3 ml-2 bg-white dark:bg-gray-900" style={{ maskImage:"url(\"https://mintlify.b-cdn.net/solid/arrow-right-long.svg\")",maskRepeat:"no-repeat",maskPosition:"center center" }} />

</button>

</a>

## Add Knowledge Base

### What is a Knowledge Base?

A Knowledge Base is a searchable repository of information that your agents can query to retrieve relevant context. It uses vector embeddings and similarity search (RAG) to find the most relevant information for a given query.

### Why use a Knowledge Base?

Knowledge Bases enable you to:
- **Ground responses in facts**: Reduce hallucinations by providing verified information
- **Work with large documents**: Process and query documents too large to fit in context
- **Dynamic information retrieval**: Automatically find relevant information without manual searching
- **Domain expertise**: Give your agents access to specialized knowledge and documentation

Add a knowledge base to provide context and information to your agent.

```bash
uv pip install "upsonic[loaders]"
# pip install "upsonic[loaders]"
```

```python with_knowledge.py
from upsonic import Agent, Task, KnowledgeBase
from upsonic.embeddings import OpenAIEmbedding
from upsonic.vectordb import QdrantProvider
from upsonic.vectordb.config import QdrantConfig, ConnectionConfig, Mode
from upsonic.loaders.markdown import MarkdownLoader

embedding_provider = OpenAIEmbedding()

config = QdrantConfig(
    connection=ConnectionConfig(
        mode=Mode.EMBEDDED,
        db_path="./upsonic_vectors"
    ),
    collection_name="upsonic_knowledge",
    vector_size=1536
)
vectordb = QdrantProvider(config)

# Explicitly provide a loader for the markdown file
loader = MarkdownLoader()

kb = KnowledgeBase(
    sources=["README.md"],
    embedding_provider=embedding_provider,
    vectordb=vectordb,
    loaders=[loader]  # Explicitly provide the loader
)

agent = Agent(model="openai/gpt-5")

task = Task(
    description="What is the telemetry information of the upsonic",
    context=[kb]
)
agent.print_do(task)
```

<a href="/concepts/knowledge-base">
<button type="button" className="px-5 flex items-center font-medium text-sm rounded-full py-2 shadow-sm text-white dark:text-gray-900 bg-primary-dark dark:bg-primary-light hover:opacity-[0.9] hover:bg-primary justify-center">
RAG and More

<svg className="size-3 ml-2 bg-white dark:bg-gray-900" style={{ maskImage:"url(\"https://mintlify.b-cdn.net/solid/arrow-right-long.svg\")",maskRepeat:"no-repeat",maskPosition:"center center" }} />

</button>

</a>

## Create Team

### What is a Team?

A Team is a collection of specialized agents that collaborate to solve complex tasks. Each agent has its own role, expertise, and responsibilities, working together like a human team.

### Why use Teams?

Teams enable you to:
- **Divide complex work**: Break down large problems into specialized sub-tasks
- **Combine expertise**: Leverage different agent strengths (research, writing, analysis, etc.)
- **Sequential workflows**: Chain agents where output of one becomes input for another
- **Parallel processing**: Run multiple agents simultaneously for faster results

Create a team of agents that work together to solve complex tasks.

```python with_team.py
from upsonic import Agent, Task, Team

researcher = Agent(
    model="openai/gpt-5",
    name="Researcher",
    role="Research Specialist",
    goal="Find accurate information and data"
)

writer = Agent(
    model="openai/gpt-5",
    name="Writer",
    role="Content Writer",
    goal="Create clear and engaging content"
)

team = Team(agents=[researcher, writer], mode="sequential")

tasks = [
    Task(description="Research the latest developments in quantum computing"),
    Task(description="Write a blog post about quantum computing for general audience")
]

result = team.do(tasks)
print(result)
```

<a href="/concepts/team">
<button type="button" className="px-5 flex items-center font-medium text-sm rounded-full py-2 shadow-sm text-white dark:text-gray-900 bg-primary-dark dark:bg-primary-light hover:opacity-[0.9] hover:bg-primary justify-center">
Learn Details

<svg className="size-3 ml-2 bg-white dark:bg-gray-900" style={{ maskImage:"url(\"https://mintlify.b-cdn.net/solid/arrow-right-long.svg\")",maskRepeat:"no-repeat",maskPosition:"center center" }} />

</button>

</a>

