...
```console
docker run --env-file .env -d -p 7341:7341 -p 7340:7340 --name upsonic_on_prem --restart=always -v upsonic_on_prem_data:/db upsonic/on-prem_without_models:v0.22.2
```
...
As you can observe, we are recommending a specific version, `v0.22.2`. This recommendation is based on our functionality testing and stability analysis.
...
```console
docker run --env-file .env -d -p 7341:7341 -p 7340:7340 --name upsonic_on_prem --restart=always -v upsonic_on_prem_data:/db upsonic/on-prem:v0.22.2
```
