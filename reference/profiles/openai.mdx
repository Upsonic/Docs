...
This function returns the model profile for an OpenAI model. It creates an OpenAIModelProfile instance with specific configurations based on the model name. For reasoning models (starting with 'o' or 'gpt-5'), it disables certain sampling settings. For search-preview models, it enables web search support. For o1-mini models, it sets the system prompt role to 'user'.
