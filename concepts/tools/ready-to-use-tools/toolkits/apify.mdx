...
**ApifyTools** extends **ToolKit** and integrates [Apify](https://apify.com/) Actors into your agents as callable tools. It inherits all ToolKit behavior (e.g. `include_tools`, `exclude_tools`, `timeout`, `use_async`). For the full ToolKit API and options, see [Creating ToolKit](/concepts/tools/custom-tools/creating-toolkit). Apify is a platform with a marketplace of ready-to-use Actors for web scraping, data extraction, search engine scraping, social media scraping, and more. Each Actor you register becomes an individual tool that the model can call with the Actor's input parameters.
