...
- For Azure OpenAI: Built-in tools are NOT supported - use custom tools instead

## Azure OpenAI Limitations

**Built-in tools (WebSearchTool, CodeExecutionTool) do NOT work with Azure OpenAI.**

- ❌ Azure OpenAI: Not supported (use custom tools instead)
- ✅ Standard OpenAI API: Full support with `openai-responses/gpt-4o`
...
# Create OpenAIResponsesModel with standard OpenAI provider
model = OpenAIResponsesModel(
    model_name="gpt-4o",
    provider="openai"  # Use standard OpenAI provider
)
...
agent = Agent(model=model, name="Web Search Agent", debug=False)
...
agent = Agent(model=model, name="Custom Search Agent", debug=False)
...
agent = Agent(model=model, name="Web Reader Agent", debug=False)
...
agent = Agent(model=model, name="Multi-URL Reader Agent", debug=False)
...
# Create task focused on web search
task = Task(
...
# Create task focused on code execution
task = Task(
    description="Write a Python function that calculates the factorial of a number and test it with the value 5",
    tools=[code_execution]
)

# Create agent with the model
agent = Agent(model=model, name="Code Execution Agent", debug=False)
...
# Web search tool with high context size and max uses
...
# Create task that combines both tools
task = Task(
...
# Create agent with the model
agent = Agent(model=model, name="Research & Code Agent", debug=False)
...
agent = Agent(model=model, name="Custom Builtin Wrapper Agent", debug=False)
...
- Custom tools: ✅ Full support with `azure/gpt-4o`
...
**"Unsupported data type" error with Azure OpenAI**
- **Solution**: Built-in tools don't work with Azure OpenAI. Use custom tools or switch to standard OpenAI API.
