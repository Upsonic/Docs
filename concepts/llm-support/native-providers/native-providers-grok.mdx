...
model = OpenAIChatModel(model_name="grok-2-vision-1212", provider="grok")
agent = Agent(model=model)
...
model = OpenAIChatModel(model_name="grok-2-image-1212", provider="grok")
agent = Agent(model=model)
...
task = Task(
    "Check the database stats and tell me if we need more storage",
