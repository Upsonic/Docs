...
model = infer_model("cohere/command-r-plus")
```
...
model = infer_model("cohere/command-r-plus")
agent = Agent(model=model, tools=[search_products])

task = Task("Find me laptops under $1000")
result = agent.do(task)
```
...
| `p` (top_p) | `float` | Nucleus sampling | 0.75 |
