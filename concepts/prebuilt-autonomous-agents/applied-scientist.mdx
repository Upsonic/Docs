...
Prepares an isolated workspace for the experiment and materializes the current notebook, data, and research source into it without touching the originals. The research source is handled based on its kind: a local PDF is copied in as `research.pdf`, other local files (Markdown, HTML, `.ipynb`, text) keep their extension as `research_source.{ext}`, GitHub/GitLab/Bitbucket repositories are shallow-cloned into `research_source/`, Kaggle notebook or dataset pages are pulled down (via the Kaggle link or API) into `research_source/`, and regular web URLs are fetched and saved as `research_source.html` (with the matching PDF pulled in too for arXiv-style links). This lets all later phases operate on their own copies without putting production code at risk.
...
Before you launch a run, you need three things ready on disk. The agent does not generate these for you, you bring them in.
...
| Your current data location | Where the baseline data comes from. It can be a file path (CSV, Parquet, etc.), a folder, or a short description of an in-notebook loader (e.g. `"downloaded in notebook (ucimlrepo, id=2)"`). |
...
You will also pick an `inputs` directory that the agent is allowed to read from. This is typically the folder that holds the three items above. Local files passed as `research_source` must live inside this tree so the agent can read them; remote sources (URLs, git repos, Kaggle pages) do not need to.
...
`research_source` is polymorphic — the agent detects its kind and materializes it inside the experiment folder before reading it:
...
| `research_source` | Reference to the method you want to try. Can be a local file (PDF, Markdown, HTML, `.ipynb`, text), a web URL, a GitHub / GitLab / Bitbucket repository URL, or a Kaggle notebook or dataset page |
...
exp = scientist.new_experiment(
    "catboost_adult",
    research_source=...,     # PDF, URL, GitHub/GitLab repo, Kaggle page, Markdown/HTML, ...
    current_notebook=...,
    current_data=...,
    experiments_directory="./experiments",
    inputs=["example_1/"],
