...
| New research paper PDF | The paper describing the method you want to try against the baseline. Supplied as a path to a `.pdf` file. |

You will also pick an `inputs` directory that the agent is allowed to read from. This is typically the folder that holds the three items above.
