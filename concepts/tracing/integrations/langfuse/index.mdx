...
- [Scores](/tracing/integrations/langfuse/advanced/scores) — Add numeric, boolean, or categorical scores to traces
- [Score Configs](/tracing/integrations/langfuse/advanced/score-configs) — Define validation rules for scores
- [Datasets](/tracing/integrations/langfuse/advanced/datasets) — Create datasets, add items, and link traces
- [Annotation Queues](/tracing/integrations/langfuse/advanced/annotation-queues) — Create review queues for human evaluation
- [Update Trace](/tracing/integrations/langfuse/advanced/update-trace) — Override trace output or metadata after a run
...
<Info>
**Why call `langfuse.shutdown()`?** Langfuse sends traces asynchronously in the background. If your script exits before the internal buffer is flushed, some traces may be lost. Calling `langfuse.shutdown()` ensures all pending traces are sent to the Langfuse server before the process terminates. Always place it at the end of your script or in a `finally` block for long-running applications.
</Info>
