...
# Create LLM client
client = LLMClient(model="openai/gpt-5")
...
def track_costs(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if hasattr(result, 'total_cost'):
            ...
            print(f"📊 Tokens: {result.total_input_token + result.total_output_token}")
...
1. **Choose the right model**: Use smaller models (gpt-5-mini) for simpler tasks
...
- Check the model being used (GPT-5 vs gpt-5-mini)
