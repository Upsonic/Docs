...
<Note>
  ...
  **What each optional group provides:**
  ...
  For other vector databases, replace `chroma` with `qdrant`, `milvus`, `weaviate`, `pinecone`, `faiss`, or `pgvector`. For other loaders, see the [Loaders documentation](/integrations/document-loaders/pypdf).
</Note>
...
<CardGroup cols={2}>
  <Card title="Vector Stores" icon="database" href="/integrations/overview#vector-stores">
