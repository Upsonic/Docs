...
memory = Memory(
    ...
    model="openai/gpt-5",            # Model for analysis/summaries
...
memory = Memory(
    ...
    summary_memory=True,
    model="openai/gpt-5-mini"  # Model for generating summaries
)
...
agent_new = Agent(model="openai/gpt-5", memory=memory_new_session)
