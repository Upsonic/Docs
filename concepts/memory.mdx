...
The `Memory` class is the core component that orchestrates all memory functionality. It works seamlessly with any storage backend and provides three types of memory management.
...
**Full Session Memory** (`full_session_memory=True`)
- Stores complete conversation history
- Requires `session_id`
- Best for: Customer support, tutoring, detailed conversations

**Summary Memory** (`summary_memory=True`) 
- Maintains evolving conversation summaries
- Requires `session_id` and `model`
- Best for: Long conversations, cost reduction

**User Analysis Memory** (`user_analysis_memory=True`)
- Learns about users across all sessions
- Requires `user_id` and `model`
- Best for: Personalization, user preferences
...
# User learning with custom schema
...
# Dynamic user learning (agent decides what to remember)
memory = Memory(
    storage=InMemoryStorage(),
    session_id="session_001", 
    user_id="user_001",
...
```python
# Update mode (default) - merges new information
memory = Memory(
    storage=storage,
    user_analysis_memory=True,
    user_memory_mode='update'  # Accumulates knowledge over time
)

# Replace mode - overwrites profile each time  
memory = Memory(
    storage=storage,
    user_analysis_memory=True,
    user_memory_mode='replace'  # Session-specific analysis
)
```
...
```python
# Production-ready memory with all features
memory = Memory(
    storage=PostgresStorage(db_url="postgresql://..."),
    session_id="prod_session_001",
    user_id="user_12345",
    full_session_memory=True,
    summary_memory=True,
    user_analysis_memory=True,
    num_last_messages=15,           # Keep last 15 conversation turns
    model="openai/gpt-4o",
    feed_tool_call_results=False,   # Exclude tool calls from memory
    debug=False
)
```
...
memory = Memory(
    ...
    summary_memory=True,
    model="openai/gpt-4o-mini",  # Model for summary generation
    debug=True  # Enable debug logging
)
