...
async def main():
    agent = Agent("openai/gpt-5-mini", name="email_agent")
    task = Task(
...
async def external_with_run_id_same_agent():
    agent = Agent("openai/gpt-5-mini", name="external_tool_agent")
    task = Task(
        description="Send an email to test@example.com with subject 'Hello' and body 'Test message'.",
        tools=[send_email]
    )
...
async def external_with_run_id_new_agent():
    ...
    agent = Agent("openai/gpt-5-mini", name="external_tool_agent", db=db)
    task = Task(
        description="Send an email to test@example.com with subject 'Hello' and body 'Test message'.",
        tools=[send_email]
    )
    ...
    new_agent = Agent("openai/gpt-5-mini", name="external_tool_agent", db=db)
...
async def external_cross_process():
    ...
    new_agent = Agent("openai/gpt-5-mini", name="external_tool_agent", db=new_db)
