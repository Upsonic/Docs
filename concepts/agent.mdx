...
**Warning**: When creating an agent without specifying a model, it defaults to `"openai/gpt-5"`. Make sure you have the appropriate API key set in your environment.
...
| Attribute | Type | Default | Description |
|-----------|------|---------|-------------|
| `model` | `Union[str, Model]` | `"openai/gpt-5"` | Model specification (string or Model instance) |
